...
Texture *MBAACC_CG::draw_texture_with_boxes(unsigned int n, unsigned int *palette, bool to_pow2_flg, RenderProperties* properties, MBAACC_Frame* frame) {
	const MBAACC_CG_Image *image = get_image(n);
	if (!image) {
		return 0;
	}
  ...
  // Rescale Image
	unsigned char *rescale_pixels = new unsigned char[width*height*4*4];
	memset(rescale_pixels, 0, width*height*4*4);
  for (int w=0; w < width; ++w) {
      for (int h=0; h < height; ++h) {
          unsigned char* p = get_pixel(w,h,width,height,pixels);
          int new_w = w*2;
          int new_h = h*2;
          set_pixel(new_w,new_h, width*2, height*2, rescale_pixels, p);
          set_pixel(new_w+1,new_h, width*2, height*2, rescale_pixels, p);
          set_pixel(new_w,new_h+1, width*2, height*2, rescale_pixels, p);
          set_pixel(new_w+1,new_h+1, width*2, height*2, rescale_pixels, p);
      }
  }
  width = width*2;
  height = height*2;
  // Draw Boxes
	...
	if (properties->display_collision_box) {
		if (frame->hitboxes[0]) {
			...
			draw_boxes(BOX_COLLISION, &rect, 1, properties->display_solid_boxes, rescale_pixels, width, height, frame);
		}
	}
	...
	if (properties->display_hit_box) {
		...
		if (nrects > 0) {
        draw_boxes(BOX_HIT, rects, nrects, properties->display_solid_boxes, rescale_pixels, width, height, frame);
		}
	}
	...
	if (properties->display_attack_box) {
		...
		if (nrects > 0) {
        draw_boxes(BOX_ATTACK, rects, nrects, properties->display_solid_boxes, rescale_pixels, width, height, frame);
		}
	}
	...
	if (properties->display_clash_box) {
		...
		if (nrects > 0) {
        draw_boxes(BOX_CLASH, rects, nrects, properties->display_solid_boxes, rescale_pixels, width, height, frame);
		}
	}
	...
	if (!texture->init(rescale_pixels, width, height, is_8bpp)) {
		delete texture;
		delete[] pixels;
		delete[] rescale_pixels;
		texture = 0;
	} else {
      texture->offset(image->bounds_x1*2, image->bounds_y1*2);
	}
...
void MBAACC_CG::draw_boxes(BoxType box_type, rect_t *rects, int nrects, bool solid, unsigned char* pixels, int width, int height, MBAACC_Frame* frame) {
  unsigned char R;
  unsigned char G;
  unsigned char B;
  unsigned char A;
  /*
  for (int i = 0; i < 512; ++i) {
      pixels[(i+(512-64-17)*width)*4] = 0xFF;
      pixels[(i+(512-64-17)*width)*4+3] = 0xFF;
      pixels[(255+(i)*width)*4] = 0xFF;
      pixels[(255+(i)*width)*4+3] = 0xFF;
  }
  */
    ...
    for (int i = 0; i < nrects; ++i) {
        ...
        for (int x=rect.x1; x<rect.x2; ++x){
            for (int y = rect.y1; y<rect.y2; ++y) {
                int new_x = x + 256-frame->AF.offset_x*2;
                int new_y = y + 512-64-frame->AF.offset_y*2;
  ...
  for (int i = 0; i < nrects; ++i) {
      ...
      for (int x=rect.x1; x<rect.x2; ++x) {
          int new_x = x + 256-frame->AF.offset_x*2;
          int new_y1 = rect.y1 + 512-64-frame->AF.offset_y*2;
          int new_y2 = rect.y2 + 512-64-frame->AF.offset_y*2;
      ...
      for (int y = rect.y1; y<rect.y2; ++y) {
          int new_x1 = rect.x1 + 256-frame->AF.offset_x*2-1;
          int new_x2 = rect.x2 + 256-frame->AF.offset_x*2-1;
          int new_y = y + 512-64-frame->AF.offset_y*2;
      ...
      int corner_x = rect.x1 + 256-frame->AF.offset_x*2-1;
      int corner_y = rect.y2 + 512-64-frame->AF.offset_y*2;
